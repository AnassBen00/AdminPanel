...
final description;
final prix;
  const EditProduct({Key key, this.id, this.nom, this.description, this.prix}) : super(key: key);
    ...
    for (int i = 0; i < brands.length; i++) {
      setState(() {
        items.insert(
            0,
            DropdownMenuItem(
                child: Text(brands[i].data['nom']),
                value: brands[i].data['nom']));
      });
    }
    return items;
  }
...
//              select category
  ...
  _getBrands() async {
    ...
    setState(() {
      ...
      _currentBrand = brands[0].data['nom'];
    });
  }
